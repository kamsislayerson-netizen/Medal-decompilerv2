...
@app.route('/')
def root():
    return 'Medal bot2 Python is alive.'
...
async def decomp(ctx, *, args=''):
    ...
    if ctx.message.attachments:
        ...
        payload_text = args.replace('--lua51', '').strip()
        if payload_text:
            bytecode = payload_text.encode('utf-8')

    if not bytecode:
        await ctx.send('Attach a bytecode file or provide Base64 text with `.decomp`.')
        return
